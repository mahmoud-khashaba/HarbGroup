...
@section('admin-content')
	<div class="admin-header">
		<h1>Service {{ $gallery->title }}</h1>
		<span class="last-update">Last change: {{$gallery->updated_at->tz('CET')->format('d M, Y, H:i\h')}}</span>
	</div>

	<div class="admin-content">
		<form style="max-width: 100%;" action="projects/update/{{ $gallery->id }}" method="post" enctype="multipart/form-data">
			{{ csrf_field() }}
			...
			@if ( count($gallery->images) )
				<div class="gallery-wrap">
					<ul class="image-list sortable cf" data-link="ajax/{{ $gallery->title }}/change-project-order">
						@foreach ($gallery->images as $image)
							<li class="items-order" data-id="{{$image->id}}">
								...
								<a href="projects/image/{{ $gallery->id }}/{{ $image->id }}" 
			...
			<a class="button remove-item" href="projects/delete/{{ $gallery->id }}">Delete gallery</a>
